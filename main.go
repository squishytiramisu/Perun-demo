...
func main() {
	// Deploy contracts.
	log.Println("1. Smartcontract feltöltése a helyi ethereum blokkláncra...")
	...
	// Setup clients.
	log.Println("2. Alice és Bob kliensének létrehozása...")
	...
	log.Println("3. Alice és Bob csatornájának megnyitása (Alice kezdeményezi)")
	appAlice := alice.OpenAppChannel(bob.WireAddress())

	appBob := bob.AcceptedChannel()
	log.Println("4. Bob elfogadja a csatorna megnyitását")

	log.Println("5. Játék kezdete! ")

	log.Println("Alice lépése.")
	appAlice.Set(2, 0)

	log.Println("Bob lépése.")
	appBob.Set(0, 0)
	...
	log.Println("Alice megnyeri a játékot.")
	log.Println("Kifizetés megkezdése.")

	// Payout.
	appAlice.Settle()
	appBob.Settle()
	log.Println("Csatorna felszámolva.")
